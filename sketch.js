...
function initStage() {
  ...
  coins   = [];
  shooters = [];
  pathfinderEnemies = []; // 💥 Reset new enemies

  generateCoins();
  generateShooters();
  generatePathfinderEnemies(); // 💥 Generate new enemies
...
function draw() {
  ...
  drawCoins();
  updateShooters();
  updatePathfinderEnemies(); // 💥 Update new enemies
  updateBullets();
...
function playerHit(){player.lives--;if(player.lives<0)player.lives=0;player.i=0;player.j=0;visitedTrail.clear();markVisited(player.i,player.j);bullets=[];blocks=[]}
...
function generateShooters(){let tries=0;while(shooters.length<1&&tries<50){const i=floor(random(cols)),j=floor(random(rows));if((i===0&&j===0)||(i===goal.i&&j===goal.j)||distance(i,j,player.i,player.j)<5){tries++;continue}
...
function generatePathfinderEnemies() {
  const count = max(1, floor(stage / 2)); // Increase with stage
  let attempts = 0;
  while (pathfinderEnemies.length < count && attempts < 50) {
    const i = floor(random(cols));
    const j = floor(random(rows));

    // Don't spawn near player start or goal
...
function drawHUD(){
  ...
  if (!hud) return;
  hud.innerHTML = `Stage: ${stage}　Coins left: ${coins.length}　Lives: ${player.lives}　｜　Sound: <b>${soundMuted ? 'OFF' : 'ON'}</b> (M)`;
}
